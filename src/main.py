...
def register_routes(app):
    ...
    def send_message():
        """
        Envia mensagem para computadores selecionados ou para uma OU.
        ...
        try:
            ...
            if request.is_json:
                ...
                target_ou = data.get("target_ou") or ""
                urgent = bool(data.get("urgent", False))
                confirmation = bool(data.get("require_read_confirmation", False))
            ...
            # FormData
                ...
                target_ou = request.form.get("target_ou") or ""
                urgent = request.form.get("urgent") == "true"
                confirmation = request.form.get("require_read_confirmation") == "true"
                sound = request.form.get("sound", "true") == "true"
                image_path = None
                ...
                logger.error("Formato de conteúdo não suportado")
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Formato de conteúdo não suportado. Use JSON ou FormData.",
                        }
                    ),
                    400,
                )
            ...
            if not title:
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Título da mensagem é obrigatório",
                        }
                    ),
                    400,
                )
            if not content and not image_path:
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Conteúdo da mensagem ou imagem é obrigatório",
                        }
                    ),
                    400,
                )
            ...
            cursor.execute(
                """
                INSERT INTO messages (title, content, image_path, sender_id, 
                                    recipients, urgent, confirmation_required, sound_enabled)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (
                    title,
            ...
            logger.info(
                f"Mensagem enviada: ID {message_id}, Destinatários: {recipients}, OU: {target_ou}"
            )
            return jsonify(
                {
                    "success": True,
            ...
            return (
                jsonify(
                    {"success": False, "message": f"Erro ao enviar mensagem: {str(e)}"}
                ),
                500,
            )
